# -*- coding: utf-8 -*-
"""
@Created at 2021/5/13 13:56
@Author: Kurt
@file:GUIFull2.py
@Desc:
alpha -> c, con=0. Showrooming dominates buy at store.
    Note that if we set c=0, we can obtain desirable result easily.
    Instead, if c!=0, we obtain desirable result by setting a high cr.
beta ->  c, con=1. Always buy at store if the utility is positive.
1-alpha-beta -> c, con.
"""
...
def find_result(DataDict):
    ...
    for c, profit_u, profit_d, price_u, price_d, \
        behavior_both, behavior_show, behavior_off in tqdm(zip(np.arange(0, 1, 0.01), DataDict['profit_u'],
                                                             DataDict['profit_d'], DataDict['price_u'],
                                                             DataDict['price_d'],
                                                             DataDict['behavior_both'], DataDict['behavior_show'],
                                                             DataDict['behavior_off'])):
        ...
        if profit_u > profit_d and price_u < price_d[0]:
            ...
            cont['behavior_show'].append(behavior_show)
    return flag, cont
...
def parse(result_dict):
    ...
    behavior_show = result_dict['behavior_show']
    result_string = "".join(["{:.3f}, \t {:.3f}, \t {:.3f}, \t {:.3f}, \t {:.3f}, \t {:.3f}, \t {}, \t {}, \t {} \n"
                            .format(s1, s2, s3[0], s3[1], s4, s5, s6, s7, s8) for s1, s2, s3, s4, s5, s6, s7, s8 in
                             zip(c, price_u, price_d, profit_u, profit_d, behavior_both, behavior_off, behavior_show)])

    final_string = "c, \t p, \t pon, \t poff, \t pi_u, \t pi_d, \t behavior_both, \t behavior_off, \t behavior_show \n" \
                   + result_string
    return final_string
...
class GUI(Frame):
    ...
    def create_widgets(self):
        ...
        self.PropShow = Scale(self, orient=HORIZONTAL, from_=0, to=1,
                                 label="Proportion of showrooming consumer alpha",
                                 resolution=0.01, length=250)
        self.PropShow.grid(row=0, column=0)
        self.PropShow.set(0.05)

        self.PropOffOnly = Scale(self, orient=HORIZONTAL, from_=0, to=1,
                                label="Proportion of the offline-only consumer beta",
                                resolution=0.01, length=250)
        self.PropOffOnly.grid(row=1, column=0)
        self.PropOffOnly.set(0.05)
    ...
    def read_params(self):
        self.fig.clear()
        alpha = float(self.PropShow.get())
        beta = float(self.PropOffOnly.get())
        if alpha + beta > 1:
